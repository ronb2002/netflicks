...
def get_labels_from_time(time):
    """Returns list of labels for each frame when inputted time when goal occurred"""
...
def create_clips():
    """Creates clips from football matches to use as training/validation/test data"""
    goal_time_lst = [] # time of goal in each clip
    for i in range(consts.NUMBER_OF_VIDEOS):
        ...
        for annotation in data["annotations"]:
            if annotation["label"] == "soccer-ball" and annotation["gameTime"][0] == "1":  # goal occurred
                game_time = annotation["gameTime"].split(" - ")[1]  # time of goal occurrence
        ...
        for goal in soccer_ball_times:
            ...
            sec_before_goal = random.randint(5, 25)  #  choosing random time for goal to occur in clip
        ...
        for not_goal in range(max(0, consts.CLIPS_PER_GAME - len(soccer_ball_times))): # creating clips with no goal
            j += 1
            output_filename = f"game_{i + 1},clip_{j}.mp4"
            while True:
                start_time = random.randint(0, consts.VIDEO_LENGTH_SECS)
                for goal in soccer_ball_times:
                    if goal + 30 >= start_time >= goal - 30:  # checking that the random clip doesn't have a goal
                        continue
                break
...
def get_clips_and_labels():
    """Load clips and labels saved on disk into objects"""
    if consts.REDO_CLIPS:  # create random goal/no-goal clips from football matches
        create_clips()
